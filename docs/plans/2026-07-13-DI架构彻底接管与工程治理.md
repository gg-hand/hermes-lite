...
**Architecture:** Container 作为唯一真相源，通过 `container.get()` / `set_instance()` 管理所有组件。路由通过 FastAPI `Depends(get_xxx)` 类型安全注入。lifespan 简化为"调 container.get() + 异步预热 + 后台 task 启动"。统一 `python -m teage-liu` 包入口，删除 51 处双重 try/except 导入样板。
...
- **当前工作目录**：`e:\Java\webser\web_app\webme\teage-liu`
- **src/ 目录**为当前源码根，Phase 3 将重命名为 `teage_liu/`
...
| 文件 | 职责 | 改动类型 |
|------|------|---------|
| `teage_liu/` (原 `src/`) | 包目录 | 重命名 |
| `teage_liu/__main__.py` | python -m teage-liu 入口 | 新增 |
| `teage_liu/__init__.py` | 包初始化 | 新增 |
...
| `teage-liu.service` | systemd 服务 | 修改：ExecStart |
...
- Modify: `.gitignore`
- Move: `teage-liu.bak.20260709-183707/` 移出工作树
...
```bash
cd e:\Java\webser\web_app\webme\teage-liu
move teage-liu.bak.20260709-183707 ..\teage-liu.bak.20260709-183707_archived
```
...
- teage-liu.bak.20260709-183707/ 移出工作树
...
## Task 13: 重命名 src/ → teage_liu/ + 创建 __main__.py + 删除遗留 __main__ 块

**Files:**
- Rename: `src/` → `teage_liu/`
- Create: `teage_liu/__init__.py`、`teage_liu/__main__.py`
- Modify: `teage_liu/app.py`（删除 __main__ 块）
- Modify: 所有引用 `src.` 的文件

**Interfaces:**
- Consumes: Task 1-12 完成
- Produces: `teage_liu/` 包目录，`python -m teage-liu` 入口

- [ ] **Step 1: 写失败测试 — python -m teage-liu 入口存在**
...
class TestPackageEntry:
    ...
    def test_teage-liu_dir_exists(self):
        """teage_liu/ 目录存在。"""
...
Expected: FAIL（teage_liu/ 不存在）

- [ ] **Step 3: 重命名 src/ → teage_liu/**
...
- [ ] **Step 4: 创建 teage_liu/__init__.py**

```python
# teage_liu/__init__.py
"""Teage Liu - 个人 AI Agent 长驻 HTTP 服务。"""
```

- [ ] **Step 5: 创建 teage_liu/__main__.py**

```python
# teage_liu/__main__.py
...
在 `teage_liu/app.py` 末尾删除 `if __name__ == "__main__":` 块（line 569-583）。
`teage_liu/server.py` 在 Task 11 已删除 __main__ 块。

- [ ] **Step 7: 更新测试中的 sys.path 路径**

所有测试文件中的 `_SRC_DIR = os.path.join(os.path.dirname(__file__), "..", "src")` 改为 `_TEAGE_DIR = os.path.join(os.path.dirname(__file__), "..", "teage-liu")`。
...
cd e:\Java\webser\web_app\webme\teage-liu
git add -A
git commit -m "refactor(structure): src/ 重命名为 teage_liu/，统一包入口

- src/ → teage_liu/，新增 __init__.py + __main__.py
- python -m teage-liu 成为唯一入口
- 删除 app.py __main__ 块（与 teage_liu/__main__.py 职责重复）
- 测试 sys.path 从 src 改为 teage-liu
...
**Interfaces:**
- Consumes: Task 13 `teage_liu/` 包目录
- Produces: 所有导入为 `from teage_liu.xxx import` 绝对路径，无 try/except 样板
...
对 `teage_liu/` 下每个文件，将双重 try/except 导入样板替换为单一绝对导入：
...
```bash
cd e:\Java\webser\web_app\webme\teage-liu
findstr /s /r /n "except ImportError" teage-liu\
...
- 28 个文件 51 处 try/except 样板改写为 from teage_liu.xxx import 绝对导入
...
**Files:**
- Modify: `desktop/src-tauri/src/sidecar.rs`（启动命令 `python src/server.py` → `python -m teage-liu`）
- Modify: `deploy.sh`、`start.sh`、`restart.ps1`、`restart.sh`、`teage-liu.service`
- Create: `tests/conftest.py`

**Interfaces:**
- Consumes: Task 13 `teage_liu/` 包目录
- Produces: 所有启动命令统一为 `python -m teage-liu`，测试 sys.path 统一配置
...
class TestDeploymentUpdated:
    ...
    def test_conftest_sets_teage-liu_path(self):
        """conftest.py 设置 teage-liu 包路径。"""
        with open(os.path.join(_ROOT, "tests", "conftest.py"), "r", encoding="utf-8") as f:
            content = f.read()
        assert "teage-liu" in content
        assert "sys.path" in content

    def test_sidecar_uses_teage-liu_module(self):
        """sidecar.rs 使用 python -m teage-liu。"""
        sidecar_path = os.path.join(_ROOT, "desktop", "src-tauri", "src", "sidecar.rs")
        if not os.path.exists(sidecar_path):
            return  # 桌面端可能不存在
        with open(sidecar_path, "r", encoding="utf-8") as f:
            content = f.read()
        assert "-m teage-liu" in content or "python\" \"-m\" \"teage-liu" in content
        assert "src/server.py" not in content

    def test_deploy_script_uses_teage-liu_module(self):
        """deploy.sh 使用 python -m teage-liu。"""
        deploy_path = os.path.join(_ROOT, "deploy.sh")
        if not os.path.exists(deploy_path):
            return
        with open(deploy_path, "r", encoding="utf-8") as f:
            content = f.read()
        assert "-m teage-liu" in content
        assert "src/server.py" not in content
```
...
```python
# tests/conftest.py
"""pytest 全局配置：确保 teage-liu 包可导入。"""
import sys
from pathlib import Path

# 确保 teage-liu 包可导入
TEAGE_ROOT = Path(__file__).parent.parent
TEAGE_PKG = TEAGE_ROOT / "teage-liu"
if str(TEAGE_ROOT) not in sys.path:
    sys.path.insert(0, str(TEAGE_ROOT))
if str(TEAGE_PKG) not in sys.path:
    sys.path.insert(0, str(TEAGE_PKG))
```
...
// 修复后
let cmd = Command::new(python).args(["-m", "teage-liu"]);
```
...
# 修复后
ExecStart=/usr/bin/python -m teage-liu
```
...
- [x] **Step 8: 验证 python -m teage-liu 启动**

Run: `python -m teage-liu --help 2>&1 | Select-Object -First 5` 或 `python -c "from teage_liu.app import app; print(app.title)"`
Expected: 正常输出（无 ImportError）

- [x] **Step 9: 验证 uvicorn teage_liu.app:app 启动**（通过 `from teage_liu.app import app` 验证）

- [x] **Step 10: Commit**

Run: `python -c "import uvicorn; uvicorn.run('teage_liu.app:app', host='127.0.0.1', port=18399)"` (后台运行 3 秒后停止)
Expected: 正常启动无报错
...
- sidecar.rs: python src/server.py → python -m teage-liu
- deploy.sh/start.sh/restart.ps1/restart.sh: 启动命令统一为 python -m teage-liu
- teage-liu.service: ExecStart 改为 python -m teage-liu
...
Task 12 (阶段 2 验证) ←── 依赖 Task 4-11
                    ↓
Task 13 (重命名 teage_liu/) ←── 依赖 Task 1-12
                    ↓
