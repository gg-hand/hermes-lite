...
- **测试 mock**：所有测试文件顶部需 `from tests._mock_deps import install_mocks; install_mocks()` 后再 import teage_liu 模块
...
| 路径 | 职责 |
|------|------|
| `teage_liu/tasks/hooks/__init__.py` | hook 包入口，导出 ScheduleHook / ScheduleHookBase / HookRegistry / 4 个 hook 类 / RetryDecision |
| `teage_liu/tasks/hooks/base.py` | ScheduleHook Protocol + ScheduleHookBase 基类 + RetryDecision dataclass |
| `teage_liu/tasks/hooks/registry.py` | HookRegistry（管理 hook 实例，按 config 开关启用） |
| `teage_liu/tasks/hooks/validate_hook.py` | ValidateHook（L1 校验层） |
| `teage_liu/tasks/hooks/catchup_hook.py` | CatchUpHook（L2 补偿层） |
| `teage_liu/tasks/hooks/retry_hook.py` | RetryHook（L3 重试层）+ `_is_permanent_error` 函数 |
| `teage_liu/tasks/hooks/notify_hook.py` | NotifyHook（L4 通知层） |
| `teage_liu/tasks/hooks/notify_templates.py` | 邮件 HTML 模板 + Webhook JSON 模板渲染 |
...
| 路径 | 修改内容 |
|------|----------|
| `teage_liu/tasks/cron_tool_loader.py` | CronToolMeta 新增 `dir_name` / `registered_name` 字段 + `get_registered_name()` 方法 |
| `teage_liu/agent/cron_tool_registry.py` | `register` 用 registered_name 作 key；`_build_handler` 用 dir_name；移除 `_normalize_name` |
| `teage_liu/tasks/scheduler.py` | `_trigger`→`_run_schedule` 重命名 + hook 调用点；新增 `_collect_components` / `_run_schedule_direct` / `_failure_counts` / `hooks` 属性 |
| `teage_liu/tasks/workflow/base.py` | WorkflowContext 新增 5 个 hook 状态字段 |
| `teage_liu/tasks/workflow/step_trace.py` | StepTrace 新增 `retry_reason` 字段 + `from_dict` 同步 |
| `teage_liu/tasks/run_summary.py` | RunSummary 新增 `retry_count` / `notified` / `notification_channels` 字段 + `from_dict` 同步 + `read_by_run_id` 方法 |
| `teage_liu/tasks/workflow/spec.py` | `ALLOWED_ON_FAILURE_ACTIONS` 移除 `"retry"` |
| `teage_liu/tasks/workflow/engine.py` | 移除 RetryBudget 调用；末尾 raise WorkflowExecutionError |
| `teage_liu/agent/tool_error.py` | 新增 `HookAbortError` / `ValidationError` / `WorkflowExecutionError` 异常类 |
| `teage_liu/tasks/workflow/validator.py` | 新增 `validate_workflow_spec` 函数（工具存在性 + step_type 校验） |
| `teage_liu/config_helpers.py` | `_CONFIG_TO_COMPONENTS` 更新注释（cron.hooks.* 不触发 Orchestrator 重建） |
| `teage_liu/routes/cron_tools.py` | 新增 5 个 API 端点（执行历史/手动重跑/单次详情/最近记录/统计） |
...
**Files:**
- Modify: `teage_liu/tasks/cron_tool_loader.py:77-122`（CronToolMeta dataclass）
- Modify: `teage_liu/agent/cron_tool_registry.py:89-130, 181-200, 334-367`（register / _build_handler / 移除 _normalize_name）
...
修改 `teage_liu/tasks/cron_tool_loader.py` 的 `CronToolMeta` dataclass（在 `run_interpreter` 字段之后新增）：
...
修改 `teage_liu/agent/cron_tool_registry.py`：
...
```bash
git add teage_liu/tasks/cron_tool_loader.py teage_liu/agent/cron_tool_registry.py tests/test_cron_tool.py
...
**Files:**
- Modify: `teage_liu/tasks/scheduler.py:1070-1190`（`_build_workflow_context` 方法）
...
在 `teage_liu/tasks/scheduler.py` 的 `CronScheduler` 类中新增（在 `_build_workflow_context` 之前）：
...
```bash
git add teage_liu/tasks/scheduler.py tests/test_scheduler.py
...
**Files:**
- Modify: `teage_liu/tasks/scheduler.py`（`_append_run_summary` 方法，搜索定位）
...
在 `teage_liu/tasks/scheduler.py` 的 `_append_run_summary` 方法中，将静默 return 改为 `logger.error` + return：
...
```bash
git add teage_liu/tasks/scheduler.py tests/test_cron_persist.py
...
**Files:**
- Create: `teage_liu/tasks/hooks/__init__.py`
- Create: `teage_liu/tasks/hooks/base.py`
- Create: `teage_liu/tasks/hooks/registry.py`
- Modify: `teage_liu/tasks/workflow/base.py:86-163`（WorkflowContext dataclass）
...
Expected: FAIL（`teage_liu.tasks.hooks.base` 模块不存在）

- [ ] **Step 3: Create hooks package with base.py**

新建 `teage_liu/tasks/hooks/__init__.py`（空文件，后续 task 会填充导出）。

新建 `teage_liu/tasks/hooks/base.py`：
...
修改 `teage_liu/tasks/workflow/base.py` 的 `WorkflowContext` dataclass（在现有最后一个 dataclass 字段之后，`ensure_report_dir` 方法之前新增）：
...
Expected: FAIL（`teage_liu.tasks.hooks.registry` 模块不存在）

- [ ] **Step 8: Create registry.py**

新建 `teage_liu/tasks/hooks/registry.py`：
...
```bash
git add teage_liu/tasks/hooks/ teage_liu/tasks/workflow/base.py tests/test_schedule_hook_base.py tests/test_hook_registry.py
...
**Files:**
- Modify: `teage_liu/agent/tool_error.py`（新增 3 个异常类）
...
在 `teage_liu/agent/tool_error.py` 末尾新增（在现有异常类之后，`__all__` 之前）：
...
```bash
git add teage_liu/agent/tool_error.py tests/test_workflow_execution_error.py
...
**Files:**
- Modify: `teage_liu/tasks/scheduler.py:545-560`（`_trigger` → `_run_schedule` 重命名 + hook 调用点）
- Modify: `teage_liu/tasks/scheduler.py:414, 1324`（`run_loop` / `trigger_now` 调用点同步重命名）
- Modify: `teage_liu/tasks/scheduler.py`（CronScheduler `__init__` 新增 `self.hooks` / `self._failure_counts`）
...
在 `teage_liu/tasks/scheduler.py` 的 `CronScheduler.__init__` 末尾新增：
...
将 `teage_liu/tasks/scheduler.py:545` 的 `async def _trigger(self, orchestrator, schedule)` 重命名为 `async def _run_schedule(self, orchestrator, schedule, started_at_dt=None)`。
...
```python
from teage_liu.agent.tool_error import HookAbortError, ValidationError, WorkflowExecutionError
```
...
```bash
git add teage_liu/tasks/scheduler.py tests/test_run_schedule_integration.py
...
**Files:**
- Create: `teage_liu/tasks/hooks/validate_hook.py`
- Modify: `teage_liu/tasks/workflow/validator.py`（新增 `validate_workflow_spec` 函数）
...
Expected: FAIL（`teage_liu.tasks.hooks.validate_hook` / `validate_workflow_spec` 不存在）

- [ ] **Step 3: Implement validate_workflow_spec in validator.py**

在 `teage_liu/tasks/workflow/validator.py` 末尾新增函数：
...
新建 `teage_liu/tasks/hooks/validate_hook.py`：
...
```bash
git add teage_liu/tasks/hooks/validate_hook.py teage_liu/tasks/workflow/validator.py tests/test_validate_hook.py
...
**Files:**
- Modify: `teage_liu/routes/cron_tools.py`（create_schedule / update_schedule 端点新增校验）
- Modify: `teage_liu/tasks/scheduler.py`（`_load_persisted` 新增启动校验）
...
在 `teage_liu/routes/cron_tools.py` 的 `create_schedule` / `update_schedule` 端点中，保存前调用 `validate_workflow_spec`，errors 非空时返回 400：
...
在 `teage_liu/tasks/scheduler.py` 的 `_load_persisted` 方法中，加载每个 schedule 后校验：
...
```bash
git add teage_liu/routes/cron_tools.py teage_liu/tasks/scheduler.py tests/test_validate_hook.py
...
**Files:**
- Modify: `teage_liu/tasks/workflow/engine.py`（移除 RetryBudget 调用 + 末尾 raise WorkflowExecutionError）
- Modify: `teage_liu/tasks/workflow/spec.py:29-31`（`ALLOWED_ON_FAILURE_ACTIONS` 移除 `"retry"`）
...
修改 `teage_liu/tasks/workflow/spec.py`：
...
在 `teage_liu/tasks/workflow/engine.py` 的 `_execute_with_policy` 方法中，移除 RetryBudget 调用，step 失败时直接按 `on_failure.action` 处理。
...
```bash
git add teage_liu/tasks/workflow/engine.py teage_liu/tasks/workflow/spec.py tests/test_workflow_engine.py tests/test_retry_budget.py
...
**Files:**
- Create: `teage_liu/tasks/hooks/retry_hook.py`
...
Expected: FAIL（`teage_liu.tasks.hooks.retry_hook` 模块不存在）

- [ ] **Step 3: Implement RetryHook + _is_permanent_error**

新建 `teage_liu/tasks/hooks/retry_hook.py`：
...
```bash
git add teage_liu/tasks/hooks/retry_hook.py tests/test_retry_hook.py
...
**Files:**
- Create: `teage_liu/tasks/hooks/catchup_hook.py`
- Modify: `teage_liu/tasks/scheduler.py`（新增 `_run_schedule_direct` thin wrapper + 启动时调用 `scan_and_compensate`）
...
Expected: FAIL（`teage_liu.tasks.hooks.catchup_hook` 不存在；`_run_schedule_direct` 不存在）

- [ ] **Step 3: Implement _run_schedule_direct in scheduler.py**

在 `teage_liu/tasks/scheduler.py` 的 `_run_schedule` 方法之后新增（如果 Task 7 未创建，则在此 Task 创建）：
...
新建 `teage_liu/tasks/hooks/catchup_hook.py`：
...
在 `teage_liu/tasks/scheduler.py` 的 `start` 方法（或 `run_loop` 启动前）中，加载 schedules 后调用：
...
```bash
git add teage_liu/tasks/hooks/catchup_hook.py teage_liu/tasks/scheduler.py tests/test_catchup_hook.py
...
**Files:**
- Modify: `teage_liu/tasks/workflow/step_trace.py:40-97`（StepTrace 新增 `retry_reason` 字段 + `from_dict` 同步）
- Modify: `teage_liu/tasks/run_summary.py:67-80`（RunSummary 新增 3 字段 + `from_dict` 同步 + `read_by_run_id` 方法）
- Modify: `teage_liu/tasks/scheduler.py`（`_run_schedule` 写入 `retry_reason` / `retry_count`）
...
修改 `teage_liu/tasks/workflow/step_trace.py`，在 StepTrace dataclass 的 `files` 字段之后新增：
...
修改 `teage_liu/tasks/run_summary.py`，在 RunSummary dataclass 的 `workflow_name` 字段之后新增：
...
修改 `teage_liu/tasks/scheduler.py` 的 `_run_schedule` 方法，在 retry 循环 break（give_up）后、`_append_run_summary` 之前：
...
```bash
git add teage_liu/tasks/workflow/step_trace.py teage_liu/tasks/run_summary.py teage_liu/tasks/scheduler.py tests/test_step_trace.py tests/test_run_summary.py tests/test_cron_persist.py
...
**Files:**
- Create: `teage_liu/tasks/hooks/notify_hook.py`
- Create: `teage_liu/tasks/hooks/notify_templates.py`
...
Expected: FAIL（`teage_liu.tasks.hooks.notify_hook` 模块不存在）

- [ ] **Step 3: Implement notify_templates.py**

新建 `teage_liu/tasks/hooks/notify_templates.py`：
...
新建 `teage_liu/tasks/hooks/notify_hook.py`：
...
```bash
git add teage_liu/tasks/hooks/notify_hook.py teage_liu/tasks/hooks/notify_templates.py tests/test_notify_hook.py
...
- Modify: `teage-liu/.env.example`（新增 SMTP_* 环境变量占位）
...
新增 `teage-liu/.env.example`（如已存在则追加）：
...
**Files:**
- Modify: `teage_liu/routes/cron_tools.py`（新增 5 个 API 端点）
...
在 `teage_liu/routes/cron_tools.py` 新增 5 个端点函数：
...
```bash
git add teage_liu/routes/cron_tools.py tests/test_cron_api_endpoints.py
