...
class TestMetricsReset:
    ...
    def test_reset_metrics_success(self, client):
        ...
        from teage_liu.server import app
        from teage_liu.app import get_metrics_collector
...
class TestMetricsPersistLoop:
    ...
    def test_first_flush_uses_short_delay(self):
        ...
        with patch("teage_liu.background_loops.load_config", return_value={"monitoring": {"flush_interval_minutes": 60}}), \
    ...
    def test_config_read_failure_uses_default(self):
        ...
        with patch("teage_liu.background_loops.load_config", side_effect=Exception("config read failed")), \
             patch("asyncio.sleep", fast_sleep), \
             patch("asyncio.to_thread", new=AsyncMock(side_effect=lambda fn, *a, **kw: fn(*a, **kw))):
