...
def test_constants_exist():
    ...
    from config_helpers import _RESTART_REQUIRED_KEYS, _RUNTIME_HOTUPDATE_MAP, _MISSING
...
def test_backup_config_no_exist(tmp_path):
    """配置文件不存在时备份不报错。"""
    from config_helpers import _backup_config
    _backup_config(str(tmp_path / "nonexistent.yaml"))


def test_atomic_write_config(tmp_path):
    """原子写入配置文件。"""
    from config_helpers import _atomic_write_config
    import yaml
