...
def test_validate_config_schema_rejects_non_dict_root():
    """非 dict 根节点拒绝。"""
    from teage_liu.config_helpers import _validate_config_schema
...
def test_check_needs_restart_detects_llm_change():
    ...
    from teage_liu.config_helpers import _check_needs_restart
...
def test_constants_exist():
    ...
    from teage_liu.config_helpers import _RESTART_REQUIRED_KEYS, _RUNTIME_HOTUPDATE_MAP, _MISSING
...
def test_backup_config_no_exist(tmp_path):
    """配置文件不存在时备份不报错。"""
    from teage_liu.config_helpers import _backup_config
    _backup_config(str(tmp_path / "nonexistent.yaml"))


def test_atomic_write_config(tmp_path):
    """原子写入配置文件。"""
    from teage_liu.config_helpers import _atomic_write_config
    import yaml
