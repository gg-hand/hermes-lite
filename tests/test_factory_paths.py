...
class TestFactoryImportPaths:
    ...
    def test_session_logger_path(self):
        ...
        from teage_liu.app import _create_session_logger
        with patch("teage_liu.storage.sqlite_log.SessionLogger") as mock_cls:
    ...
    def test_metrics_store_path(self):
        ...
        from teage_liu.app import _create_metrics_store
        with patch("teage_liu.monitoring.metrics_store.MetricsStore") as mock_cls:
    ...
    def test_audit_logger_path(self):
        ...
        from teage_liu.app import _create_audit_logger
        with patch("teage_liu.agent.audit.AuditLogger") as mock_cls:
    ...
    def test_approval_manager_path(self):
        ...
        from teage_liu.app import _create_approval_manager
        with patch("teage_liu.agent.approval.ApprovalManager") as mock_cls:
    ...
    def test_stream_manager_path(self):
        ...
        from teage_liu.app import _create_stream_manager
        with patch("teage_liu.stream_manager.StreamManager") as mock_cls:
            _create_stream_manager()
            mock_cls.assert_called_once()

    def test_skill_loader_path(self):
        """_create_skill_loader 应从 skill.loader 导入。"""
        from teage_liu.app import _create_skill_loader
        with patch("teage_liu.skill.loader.SkillLoader") as mock_cls:
            _create_skill_loader()
            mock_cls.assert_called_once()

    def test_proposal_store_path(self):
        """_create_proposal_store 应从 agent.cron_proposals 导入。"""
        from teage_liu.app import _create_proposal_store
        with patch("teage_liu.agent.cron_proposals.ProposalStore") as mock_cls:
            _create_proposal_store()
            mock_cls.assert_called_once()

    def test_mcp_manager_path(self):
        """_create_mcp_manager 应从 mcp.manager 导入。"""
        from teage_liu.app import _create_mcp_manager
        with patch("teage_liu.mcp.manager.MCPManager") as mock_cls:
    ...
    def test_etl_engine_paths(self):
        ...
        with patch("teage_liu.files.parser.WaterfallParser") as mock_parser, \
             patch("teage_liu.files.chunker.DocumentChunker") as mock_chunker, \
             patch("teage_liu.files.etl_engine.ETLEngine") as mock_etl:
            mock_orch = MagicMock()
    ...
    def test_cron_scheduler_path(self):
        ...
        from teage_liu.app import _create_cron_scheduler
        with patch("teage_liu.tasks.scheduler.CronScheduler") as mock_cls:
...
class TestHealthCheckerFactory:
    ...
    def test_health_checker_receives_six_args(self):
        ...
        from teage_liu.app import _create_health_checker
        with patch("teage_liu.monitoring.health.HealthChecker") as mock_cls:
            mock_orch = MagicMock()
