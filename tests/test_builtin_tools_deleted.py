...
删除后：
- `import hermes.agent.builtin_tools` 应失败（ModuleNotFoundError，ImportError 子类）
- `src.agent.tools` 仍提供 register_builtin_tools / BUILTIN_TOOLS
- test_plan_tools.py 中 `from hermes.agent import builtin_tools` 改为别名 `tools as builtin_tools`
"""
...
def test_builtin_tools_module_not_importable():
    """hermes.agent.builtin_tools 模块应已删除，import 失败。"""
    with pytest.raises(ImportError):
        importlib.import_module("hermes.agent.builtin_tools")


def test_tools_module_still_provides_register_builtin_tools():
    """hermes.agent.tools 仍提供 register_builtin_tools。"""
    from hermes.agent.tools import register_builtin_tools, BUILTIN_TOOLS
...
def test_tools_module_still_provides_plan_tools_imports():
    """hermes.agent.tools.plan_tools 仍可正常导入 register_plan_tools。"""
