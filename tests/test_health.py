...
from teage_liu.monitoring.health import (  # noqa: E402
    CheckResult,
    HealthChecker,
    HealthSummary,
)
...
class TestHealthCheckerLogging(unittest.TestCase):
    ...
    def test_healthy_no_warning_logs(self):
        ...
        logger = logging.getLogger("teage_liu.monitoring.health")
        buf = io.StringIO()
    ...
    def test_warning_produces_log(self):
        ...
        checker = HealthChecker(orchestrator=mock_o)
        with self.assertLogs("teage_liu.monitoring.health", level="WARNING") as cm:
            checker.run_all()
        self.assertTrue(
