...
from teage_liu.agent.skill_tools import register_skill_tools  # noqa: E402
from teage_liu.skill.loader import Skill, SkillLoader  # noqa: E402
...
class TestProposeSkill(TestCase):
    ...
    def _propose(self, tools_input=None, patch_base=True):
        ...
        if patch_base:
            with patch("teage_liu.agent.skill_tools.SKILL_BASE_DIR", self.temp_dir):
    ...
    def test_propose_existing_on_disk_via_skill_loader_dir(self):
        ...
        try:
            with patch("teage_liu.agent.skill_tools.SKILL_BASE_DIR", other_temp):
...
class TestToggleSkill(TestCase):
    ...
    def test_disable_returns_disabled_status(self):
        ...
        try:
            with patch("teage_liu.agent.skill_tools.SKILL_STATE_PATH", Path(state_path)):
                result = self.registry.execute_tool(
    ...
    def test_toggle_state_file_not_found_uses_default(self):
        ...
        with patch(
            "teage_liu.agent.skill_tools.SKILL_STATE_PATH",
...
class TestListSkills(TestCase):
    ...
    def test_list_overview_with_discovered_skills(self):
        ...
        from teage_liu.skill.loader import SkillMeta
        meta = SkillMeta(
            name="disk_skill",
            version="0.2.0",
    ...
    def test_list_overview_without_skill_loader_falls_back_to_disk_scan(self):
        ...
        try:
            with patch("teage_liu.agent.skill_tools.SKILL_BASE_DIR", tmp_skills):
