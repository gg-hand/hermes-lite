...
def _collect_src_modules() -> list:
    ...
    返回:
        模块全限定名列表，如 ["src.config", "src.llm.client", ...]，
        按 ASCII 字典序排列。
    """
...
class TestImports(unittest.TestCase):
    ...
    def test_src_modules_collected(self):
        ...
        expected_modules = [
            ...
            "src.memory.context_manager",
            "src.memory.condenser",
            "src.memory.decay",
            "src.storage.chroma_store",
            "src.storage.history_buffer",
            "src.storage.sqlite_log",
        ]
