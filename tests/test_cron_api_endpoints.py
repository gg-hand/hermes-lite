"""cron API 端点测试（7.3）。

路由已迁移为 Depends(get_cron_scheduler) DI 注入，测试直接传 cron_scheduler 参数。
"""
from __future__ import annotations

import sys
import unittest
from pathlib import Path
from unittest.mock import MagicMock, AsyncMock

_PROJECT_ROOT = str(Path(__file__).resolve().parent.parent)
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from tests._mock_deps import install_mocks
install_mocks()

# 先 import teage_liu.server.app 触发完整 app 装配，避免循环导入
from teage_liu.server import app  # noqa: E402,F401


class TestCronRunsApiEndpoints(unittest.TestCase):
    """5 个 API 端点：执行历史 / 手动重跑 / 单次详情 / 最近记录 / 统计。"""

    def setUp(self):
        self.cron_scheduler = MagicMock()
        self.cron_scheduler.runs_store = MagicMock()
        # mock schedule 列表（用于 trigger 与 read_recent_all）
        self.mock_schedule = MagicMock()
        self.mock_schedule.id = "s1"
        self.mock_schedule.name = "test_schedule"
        self.cron_scheduler.list_schedules.return_value = [
            {"id": "s1", "name": "test_schedule"}
        ]
        # _schedules 用于 trigger_schedule_run 中的查找
        self.cron_scheduler._schedules = [self.mock_schedule]

    def test_get_schedule_runs_returns_list(self):
        """GET /cron_tools/schedules/{id}/runs 返回执行历史列表。"""
        from teage_liu.tasks.run_summary import RunSummary
        mock_runs = [
            RunSummary(schedule_id="s1", run_id="r1", success=True),
            RunSummary(schedule_id="s1", run_id="r2", success=False),
        ]
        self.cron_scheduler.runs_store.read_recent.return_value = mock_runs

        from teage_liu.routes.cron_tools import get_schedule_runs
        response = get_schedule_runs("s1", limit=20, cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 200)

    def test_get_schedule_runs_scheduler_none_returns_503(self):
        """scheduler 为 None 时返回 503。"""
        from teage_liu.routes.cron_tools import get_schedule_runs
        response = get_schedule_runs("s1", limit=20, cron_scheduler=None)
        self.assertEqual(response.status_code, 503)

    def test_get_run_by_id_returns_detail(self):
        """GET /cron_tools/schedules/{id}/runs/{run_id} 返回单次详情。"""
        from teage_liu.tasks.run_summary import RunSummary
        mock_run = RunSummary(schedule_id="s1", run_id="r1", success=False)
        mock_dict = mock_run.to_dict()
        self.cron_scheduler.runs_store.read_by_run_id.return_value = mock_dict

        from teage_liu.routes.cron_tools import get_run_detail
        response = get_run_detail("s1", "r1", cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 200)

    def test_get_run_by_id_not_found_returns_404(self):
        """run_id 不存在时返回 404。"""
        self.cron_scheduler.runs_store.read_by_run_id.return_value = None

        from teage_liu.routes.cron_tools import get_run_detail
        response = get_run_detail("s1", "nonexistent", cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 404)

    def test_trigger_run_returns_202(self):
        """POST /cron_tools/schedules/{id}/run 异步触发，返回 202。"""
        from teage_liu.routes.cron_tools import trigger_schedule_run
        # mock _run_schedule_direct 为 AsyncMock 避免实际执行
        self.cron_scheduler._run_schedule_direct = AsyncMock()
        response = trigger_schedule_run("s1", cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 202)

    def test_trigger_run_schedule_not_found_returns_404(self):
        """schedule_id 不存在时返回 404。"""
        from teage_liu.routes.cron_tools import trigger_schedule_run
        response = trigger_schedule_run("nonexistent", cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 404)

    def test_get_recent_runs_returns_all_schedules(self):
        """GET /cron_tools/runs/recent 返回所有调度最近记录。"""
        from teage_liu.routes.cron_tools import get_recent_runs
        self.cron_scheduler.runs_store.read_recent_all.return_value = []
        response = get_recent_runs(limit=50, cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 200)

    def test_get_run_stats_returns_summary(self):
        """GET /cron_tools/runs/stats 返回今日统计。"""
        from teage_liu.routes.cron_tools import get_run_stats
        self.cron_scheduler.runs_store.read_recent_all.return_value = []
        response = get_run_stats(cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 200)

    def test_get_run_stats_with_data(self):
        """stats 端点正确统计今日成功/失败/耗时。"""
        from datetime import date
        today_str = date.today().isoformat()
        mock_runs = [
            {"started_at": today_str + "T10:00:00", "success": True, "duration_seconds": 5.0},
            {"started_at": today_str + "T11:00:00", "success": False, "duration_seconds": 3.0},
            {"started_at": "2020-01-01T00:00:00", "success": True, "duration_seconds": 1.0},  # 非今日
        ]
        self.cron_scheduler.runs_store.read_recent_all.return_value = mock_runs

        from teage_liu.routes.cron_tools import get_run_stats
        import json
        response = get_run_stats(cron_scheduler=self.cron_scheduler)
        self.assertEqual(response.status_code, 200)
        body = json.loads(response.body)
        self.assertEqual(body["today_success"], 1)
        self.assertEqual(body["today_failure"], 1)
        self.assertEqual(body["today_total"], 2)
        self.assertEqual(body["total_duration_seconds"], 8.0)


if __name__ == "__main__":
    unittest.main()
