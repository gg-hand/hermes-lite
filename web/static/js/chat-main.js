...
function bindEvents() {
  ...
  // 协作工作台按钮：打开独立 /workbench 页面（新标签页）
  const btnOpenWorkbench = document.getElementById('btnOpenWorkbench');
  if (btnOpenWorkbench) {
  ...
  // Task 10：协作消息不再侵入主对话 #messages（旧的 Director 启停按钮 / 分派任务对话框 / 任务块时间线 已全部移除）
  // Director 健康状态由 multiagent-sse.js + multiagent-render.js 处理 nav-dot 显示
  // 协作工作台已独立为 /workbench 页面，chat 页面通过 chat-collab-bridge.js 监听协作消息注入主对话
  // 旧的 multiagent-message / multiagent-task-status / multiagent-agent-message 事件不再在主对话渲染
  // 旧的 wb-director-control（启动/停止/重启按钮）已移除
  // 旧的 msgLogToggle / msgLogContainer 已从工作台删除（历史消息查看由 collab-workbench.js 的 loadRecentMessages() 处理）
