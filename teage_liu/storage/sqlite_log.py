...
class SessionLogger:
    ...
    - 与 :class:`~teage_liu.storage.history_buffer.HistoryBuffer` 的关系：
    ...
    def _init_tables(self):
        ...
        with self._lock:
            ...
            # 兼容旧库：若 messages 表无 reasoning 列则补加（LLM 思考内容）
            try:
                self.conn.execute("SELECT reasoning FROM messages LIMIT 1")
            except sqlite3.OperationalError:
                self.conn.execute(
    ...
    def log_message(
        ...
        Args:
            ...
            reasoning: LLM 思考内容（reasoning/thinking），仅 assistant 消息
        """
        now = self._now_iso()
        with self._lock:
            cur = self.conn.execute(
                ...
                INSERT INTO messages
                    (session_id, role, content, tool_name, tool_call_id,
                     token_count, is_error, created_at, attachments,
                ...
                (
                    ...
                    attachments, message_type, reasoning,
                ),
            )
